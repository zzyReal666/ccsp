...
import com.spms.common.pool.c3p0Pool.DataSource;
import com.spms.common.pool.c3p0Pool.DynamicDataSourcePool;
...
public class DbConnectionPoolFactorybak {
    ...
    public void bind(DbInstancePoolKeyDTO key, javax.sql.DataSource dataSourcePool){
        if (IsBePool(key)) {
            DbConnectionPool.destroy(getDynamicDataSourcePool(key));
        }
     ...
     * @return
     */
    public javax.sql.DataSource getDynamicDataSourcePool(DbInstancePoolKeyDTO key) {
        if (!IsBePool(key)) {
            return null;
        }
    ...
    public  Connection getConnection(DbInstanceGetConnDTO instance) throws ZAYKException, SQLException {
    ...
    public static String buildDataSourcePool(DbInstanceGetConnDTO instanceGetConnDTO) throws ZAYKException, SQLException {
        ...
        if (dataSource == null) {
            log.info("数据源对象为空，请检查数据库");
            throw new ZAYKException("数据源对象为空，请检查数据库");
        }
